...
class Node:
    ...
    def is_dominated(self, node):
        if self is not node and node.l >= self.l:
            for self_p, node_p in zip(self.p, node.p):
                if node_p >= self_p:
                    return True
            return False
...
def a_star(X, Y, P, alphabet):
    ...
    while Q:
        ...
        best_node = better_node(best_node, current_node)
        ...
        for child in children:
            ...
            if child_key not in N:
                ...
                is_dominated = False
                for related_node in N[child_key]:
                    if child.is_dominated(related_node):
                        # print(f"{related_node} dominates {child}")
                        is_dominated = True
                        break


                if not is_dominated:
                    N[child_key].append(child)
                    heapq.heappush(Q, child)
                    
                    # Remove other node that doesn't dominate the child from N
                    for related_node in N[child_key]:
                        if related_node.is_dominated(child):
                            N[child_key].remove(related_node)
                            saved_list = list(Q)
                            if (related_node in saved_list):
                                saved_list.remove(related_node)
                                heapq.heapify(saved_list)
                                Q = saved_list
...
def main():
    ...
    directory = "C:/Users/Anel/Desktop/AStarLCS/Dp_optimal_sequences2"
    print(directory)

    # For every instance:
    
    for file_name in os.listdir(directory):
        if file_name == 'resultsH1.txt':
        ...
        if file_name.endswith(".txt"):
            ...
            with open(os.path.join(directory, 'resultsH1.txt'), 'a') as results_file:
                if solution != None:
                    results_file.write(f"File: {file_name}, Solution: {solution.subsequence}, Position: {solution.position_vector}, Length: {solution.l}, F: {solution.f}, Time: {elapsed_time}\n")            
                else:
